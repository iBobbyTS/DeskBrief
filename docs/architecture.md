...
- Week, month, and year heatmaps share one optional daily-time overlay renderer. It splits events at day boundaries, then pre-aggregates non-overlapping depth segments per category and time interval. Every regular occurrence contributes `1 / regular activity recorded days`; Away occurrences contribute `1 / Away recorded days`. After linear addition, all regular-category segments are normalized together against their maximum raw depth, while Away is normalized separately against its own maximum. Each final segment is drawn once, so duration and layered alpha composition do not affect depth. The preserved Other category and Away are ordered last in legends and heatmap rows; bar charts omit Away bars but keep Away in the legend.
