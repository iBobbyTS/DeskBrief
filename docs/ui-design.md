...
- Week, month, and year heatmaps expose the same Overlay Daily Time switch. In overlay mode, every regular occurrence contributes `1 / regular activity recorded days` depth and every Away occurrence contributes `1 / Away recorded days`. Contributions at the same category and time interval are added linearly; regular categories are then normalized together to a maximum depth of `1`, while Away is normalized independently to its own maximum of `1`. Final segments are rendered once instead of relying on layered alpha composition or duration normalization.
